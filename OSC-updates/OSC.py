...
v0.3.7 - 3 April 2019 (AUSTIN POHLMAN)
    Added Python3 support for basic OSCclient and OSCserver
        Functionality includes sending/recieving single messages containing no data, only addresses. To be used with PureData OSC implementation.
    2to3 Function was run (2to3 -w <filename>), then OSCString(next), _readString(data), decodeOSC(data), getRegEx(pattern), and getUrlStr(*args) were updated for compatibility.
...
def OSCBlob(next):
	...
	if type(next) == str:
		OSCblobLength = int(math.ceil((len(next)) / 4.0) * 4.0)
		binary = struct.pack(">i%ds" % (OSCblobLength), OSCblobLength, bytes(str(next), 'utf-8'))
	else:
		binary = ""
...
def decodeOSC(data):
	...
	if address == "#bundle":
		...
		if typetags.startswith(b","):
            #print(typetags.decode())
			typetags = typetags.decode()
...
def getRegEx(pattern):
	...
	pattern = pattern.decode('UTF-8').replace(".", r"\.")		# first, escape all '.'s in the pattern.
