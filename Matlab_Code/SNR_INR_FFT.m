...
Data     = csvread('Input_Signal_R_5MS_s.csv',9,1,[9 1 500008 1]);
n        = length(Data);
fs       = 1/2E-7;
T        = 2E-7;
...
%plot(time,Data(4009:9008),'r')
xlabel('Time (s)')
ylabel('Amplitude (V)')
title('Time Domain Plot of 1kHz test tone')
...
plot(f(3:201),mag2db(P1(3:201)),'r')
xlabel('Frequency (Hz)')
ylabel('Gain (dB)')
title('FFT of Input using a 1kHz test tone')
%subplot(3,1,3)
...
%Now do calculations
    %1. Calculate 
E_N = 0.0058; %From NOISE_OUT_FFT
E_S = sum(P1(96:106).^2)*10
